...
function App() {
  ...
  const visibleProducts = useMemo(() => sortByFavorites(products.filter((product) => {
    const matchesCategory = category === 'Todos' || product.category === category;
    const matchesSearch = product.name.toLocaleLowerCase('pt-BR').includes(search.toLocaleLowerCase('pt-BR'));
    return matchesCategory && matchesSearch;
  }), favoriteProductIds), [category, favoriteProductIds, search]);
  ...
  function toOffers() { document.getElementById('offers')?.scrollIntoView({ behavior: 'smooth' }); }
  ...
  return <>
    <header className="site-header">
      ...
      <nav aria-label="Navegação principal"><button onClick={toProducts}>Produtos</button><button onClick={toOffers}>Ofertas</button></nav>
      <button className="cart-button" onClick={openCart} aria-label={`Carrinho, ${cartCount} ${cartCount === 1 ? 'item' : 'itens'}`}>Carrinho <span>{cartCount}</span></button>
    </header>

    <main id="top">
      <Hero onShowProducts={toProducts} onShowOffers={toOffers} />
      ...
      <section className="catalog" id="products" aria-labelledby="catalog-title">
        <div className="catalog__heading"><div><p className="section-label">Seleção TechZone</p><h2 id="catalog-title">Monte um setup à sua altura.</h2></div><label className="search"><span>Buscar produto</span><input value={search} onChange={(event) => { setSearch(event.target.value); if (event.target.value.trim().length > 1) pushSearch(event.target.value.trim()); }} placeholder="Busque por mouse, teclado, monitor..." /></label></div>
        ...
        {!visibleProducts.length ? <div className="empty-state"><h3>Nenhum produto encontrado.</h3><p>Tente outro termo ou veja toda a seleção TechZone.</p><button className="button button--secondary" onClick={() => { setSearch(''); chooseCategory('Todos'); }}>Limpar filtros</button></div> : <div className="product-grid">{visibleProducts.map((product) => {
          ...
          return <article className="product-card" key={product.id}>
            ...
            <div className="product-card__actions"><button className="button button--secondary" onClick={() => showProduct(product)}>Detalhes</button><button className="button button--primary" onClick={() => addProduct(product)}>Adicionar</button></div>
          </article>;
        })}</div>}
      </section>
      <section className="offer-band" id="offers"><p className="section-label">Ofertas selecionadas</p><h2>Desempenho de ponta, escolhas certeiras.</h2><p>{offers.length} produtos com condições especiais para elevar seu setup.</p><button className="button button--inverse" onClick={toProducts}>Ver seleção</button></section>
    </main>
