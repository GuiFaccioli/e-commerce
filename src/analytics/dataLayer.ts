...
export function mapProductToAnalyticsItem(product: Product | CartItem, quantity = 'quantity' in product ? product.quantity : 1): AnalyticsItem {
  return {
    ...
    ...(product.selectedColor ? { item_variant: product.selectedColor } : {}),
...
export const pushSelectItem = (product: Product) => pushToDataLayer({ event: 'select_item', ecommerce: { currency: 'BRL', value: priceOf(product), items: [mapProductToAnalyticsItem(product)] } });
export const pushSelectItemVariant = (product: Product) => pushToDataLayer({ event: 'select_item_variant', ecommerce: { currency: 'BRL', value: priceOf(product), items: [mapProductToAnalyticsItem(product)] } });
...
export const pushFilterProducts = (category: string, searchTerm: string, resultsCount: number) => pushToDataLayer({
  event: 'filter_products',
  filter_category: category === 'Todos' ? 'all' : category,
  filter_term: searchTerm,
